...
init_idt () {
	...
	for(index = 0; index < NUM_VEC; index++){
		...
		idt[index].seg_selector = KERNEL_CS;	//Define segment selector as the Kernel CS
		
		if(index >= 32){
			idt[index].reserved3 = 0x0;							//All vecotrs greater than 32 are initialized 		
			SET_IDT_ENTRY(idt[index], general_interruption);	//as interruptswith types 01111 and with a 
		}														//general handler for now
	}
	...
	SET_IDT_ENTRY(idt[33], keyboard_handler);	//Keyboard interrupt routed to asm wrapper
	SET_IDT_ENTRY(idt[40], clock_handler);		//Clock interrupt routed to asm wrapper
}
